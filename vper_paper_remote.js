...
var check_end_timer;
// 配置抓取、发送邮件、检测终止的时钟间隔
var wait_feed_waiting_task_timeout = 10 * 1000;  // 留给 feed_waiting_task() 执行的时间
var crawl_interval = 10 * 1000;  // 每 10 秒执行一条任务
var send_info_interval = 1000 * 60 * 30;  // 每 30 分钟发送一次统计信息
if (run_mode == 'debug') send_info_interval = 1000 * 10;  // 如果是 debug 模式，每隔 10 秒显示统计信息
var check_end_interval = 2 * 1000;  // 每 2 秒检查程序是否应该终止
// debug 模式下测试的条目数量
var debug_count = 5;
// 配置因错误种植程序的阀值，参考值是任务总量的 50%
var crawl_error_end_count = 20000;
var parse_error_end_count = 20000;
var save_error_end_count = 20000;
// 执行任务是否需要先登录
var need_login_first = true;
// 如果需要先登录，配置以下信息
var login_url = 'http://113.105.70.79:8000/auth/login.do';
var login_form = {
	// 不要包含验证码字段
	url: '',
	mac: '',
...
var login_success_flag = '0440050145638';  // 登录后成功页面包含的可供判断的字符串
var verifycode_timeout = 0;  // 验证码有效时间，如果为 0，则表示马上提交表单
// 如果需要验证码，配置以下信息
var need_verifycode = true;
var verifycode_url = 'http://113.105.70.79:8000/servlet/verifycode';  // 验证码生成的网址
...
function generate_all_tasks() {
	...
	// 由于需用到回调，在 feed_waiting_task() 直接计算
}
...
function generate_request_options(task) {
	...
	// 注意：task 有可能本身就是 url
	var url = 'http://113.105.70.79:8000/rewriter/VERS/http/udqr9bpuho9bnl/UI/down.aspx?id=' + task + '&IsAnswer=1&name=' + task;
...
// 如果保存成功，success + 1
function parse_and_save(buf, task) {
...
function feed_waiting_tasks() {
	...
	redis.smembers('vper:exam_id', function(err, exams) {

		var tasks = [];

		_.each(exams, function(exam) {
			tasks.push(exam.slice(7));
		});
	...
	redis.spop(rfs.waiting, function(err, task) {
		...
		if (task) {
			...
			ws.on('close', function() {
				// 成功保存后 success + 1，读取文件大小
				redis.incr(rfs.success);
				fs.stat(file_name, function(err, stats) {
					if (err) {
						console.log('读取文件信息错误 -> %s', file_name);
					} else {
						// console.log('保存成功 -> 任务：%s -> 文件大小：%s', task, String(stats.size));
					}
				});
			});

			// 配置 request 选项，抓取并处理数据
			...
			req.on('response', function(res) {
				if (res.statusCode == 200) {
					...
					// 如果抓取异常，则将 task 放回 waiting，同时 crawl_error + 1
					console.log('抓取 - 异常 -> %s -> %s -> 代码：%s', task, request_options.url, res.statusCode);
					redis.sadd(rfs.waiting, task, function(err, reply) {
						redis.incr(rfs.crawl_error);
						// console.log('任务数据 - 已放回 -> %s -> %s', task, reply);
					});
...
if (run_mode == 'debug') {
	redis.del(rfs.waiting, rfs.running, rfs.success, rfs.crawl_error, rfs.parse_error, rfs.save_error, rfs.result, function(err, reply) {
		...
		feed_waiting_tasks();
		// 开始执行，因为填充数据需要时间，等待 10000 毫秒
		setTimeout(start_consume, wait_feed_waiting_task_timeout);
	});
}
				...
				if (running_count > 0) {
					...
					redis.hkeys(rfs.running, function(err, running_jobs) {
						redis.sadd(rfs.waiting, running_jobs, function(err, reply) {
							// 放回去以后记得删除原来的 running 和错误状态数据
							redis.del(rfs.running, rfs.crawl_error, rfs.parse_error, rfs.save_error, function() {
								console.log('发现以前曾经运行但未完成的任务 %d 条，已放回任务列表。', running_jobs.length);
								redis.scard(rfs.waiting, function(err, reply) {
									console.log('任务总量：%s，开始执行。', reply);
									send_mail(task_name + ' 继续开始执行', '任务总量：' + reply);
									start_consume();
								});
							});
						});
					...
					// 不存在已运行但未完成的任务，直接继续开始，记得先删除原来的错误状态数据
					redis.del(rfs.crawl_error, rfs.parse_error, rfs.save_error, function() {
						redis.scard(rfs.waiting, function(err, reply) {
							console.log('任务总量：%s，开始执行。', reply);
							send_mail(task_name + ' 继续开始执行', '任务总量：' + reply);
							start_consume();
